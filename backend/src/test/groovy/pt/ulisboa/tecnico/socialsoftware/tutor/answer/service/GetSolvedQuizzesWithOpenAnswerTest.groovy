...
@DataJpaTest
class GetSolvedQuizzesWithOpenAnswerTest extends SpockTest {
    def user
    def courseDto
    def question
    def quiz
    ...
    def "returns solved quiz with: quizType=#quizType | conclusionDate=#conclusionDate | resultsDate=#resultsDate"() {
        ...
        def answerDetails = new OpenAnswer(questionAnswer)
        answerDetails.setAnswer(OPEN_QUESTION_1_ANSWER)
        ...
        answer.getAnswerDetails().getAnswer() == OPEN_QUESTION_1_ANSWER
        solvedQuizDto.getCorrectAnswers().size() == 1
        def correct = solvedQuizDto.getCorrectAnswers().get(0)
        correct.getCorrectAnswerDetails().getCorrectAnswer() == OPEN_QUESTION_1_ANSWER
        def quizAnswerRep = quizAnswerRepository.findAll().get(0)
        quizAnswerRep.numberOfCorrectAnswers == 1
    ...
    @Unroll
    def "returns solved quiz without correct answer"() {
        given: 'a quiz answered by the user'
        quiz = new Quiz()
        quiz.setKey(1)
        quiz.setTitle(QUIZ_TITLE)
        ...
        def answerDetails = new OpenAnswer(questionAnswer)
        answerDetails.setAnswer(OPEN_QUESTION_2_ANSWER)
        questionAnswer.setAnswerDetails(answerDetails)
